...
function retrieveSourceMap(source, position) {
  ...
  if (reSourceMap.test(sourceMappingURL)) {
    ...
    source = sourceMapData.sources[0].replace(/\?.*$/, '')
...
function getEvalSource(origin) {
  ...
  if (match) {
    ...
    return position.source.replace(/\?.*$/, '')
  }

  // Parse nested eval() calls using recursion
  match = /^eval at ([^(]+) \((.+)\)$/.exec(origin);
  if (match) {
    return getEvalSource(match[2]);
  }
...
function wrapCallSite(frame) {
  ...
  if (origin) {
    source = getEvalSource(origin);
    frame = cloneCallSite(frame);
    frame.getScriptNameOrSourceURL = function() { return source; };
    frame.getEvalOrigin = function() { return origin; };
    frame.getFileName = function () { return source}
    return frame;
  }
